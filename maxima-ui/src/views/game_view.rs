...
fn skeleton_text_block1(ui: &mut egui::Ui, width: f32, width1: f32, height: f32) {
    ...
    ui.painter().rect_filled(rect, Rounding::same(2.0), SKELETON_INFO_COLOR);
    rect.min.x = rect.max.x + ui.spacing().item_spacing.x;
    rect.set_width(width1);
    ui.painter().rect_filled(rect, Rounding::same(2.0), SKELETON_TEXT_COLOR);
    ui.allocate_space(vec2(width + width1 + ui.spacing().item_spacing.x, height));
}

fn game_view_action_button(text: String, ui: &mut Ui) -> bool {
    egui::Button::new(egui::RichText::new(text).size(20.0).color(Color32::WHITE))
...
fn game_view_action_buttons(app: &mut MaximaEguiApp, game: &GameInfo, ui: &mut Ui) {
    let localization = &app.locale.localization.games_view.main;
    ...
    buttons_frame.show(ui, |buttons| {
        ...
        buttons.horizontal(|buttons| {
            ...
            if app.playing_game.as_ref().is_some_and(|slug| slug.eq(&game.slug)) {
                let play_str = format!("  {}  ", &localization.stop.to_uppercase());
...
fn game_view_system_requirements(
...
pub fn game_view_details_panel(app: &mut MaximaEguiApp, ui: &mut Ui) {
    ...
    let aspect_ratio: f32 = if let Some(handle) = &hero {
        handle.aspect_ratio()
    } else {
        16.0 / 9.0
    };
    ...
    ui.vertical(|ui| {
        ...
        ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                ...
                // how much of the hero image is visible
                let mut hero_vis_frac =
                    (content_region_start - hero_rect.min.y) / (hero_rect.max.y - hero_rect.min.y);
                // clamping/inverting
                hero_vis_frac = if hero_vis_frac > 1.0 {
                    0.0
                } else if hero_vis_frac < 0.0 {
                    1.0
                } else {
                    1.0 - hero_vis_frac
                };
                ...
                if let Some(handle) = hero {
                    ...
                    if let Some(gvbg) = &app.game_view_bg_renderer {
                        gvbg.draw(
                            ...
                            handle.id(),
                            hero_vis_frac,
                            app.settings.performance_settings,
                        );
                    }
                } else {
                ...
                play_bar_frame.show(ui, |ui| {
                    ui.vertical(|ui| {
                        ...
                        stats_frame.show(ui, |stats| {
                            ...
                            stats.horizontal(|stats| {
                                ...
                                if let Some(details) = &game_details {
                                    stats.label(
                                        RichText::new(
                                            &app.locale.localization.games_view.main.playtime,
                                        )
                                        .color(Color32::BLACK)
                                        .strong(),
                                    );
                                    ...
                                    stats.label(
                                        RichText::new(
                                            &app.locale.localization.games_view.main.achievements,
                                        )
                                        .color(Color32::BLACK)
                                        .strong(),
                                    );
                        ...
                        game_view_action_buttons(app, &game, ui);
                    });
                });
                ui.vertical(|ui| {
                    ...
                    game_view_system_requirements(&app.locale, game_details, ui);

                    ui.horizontal_wrapped(|dlc| {
                        puffin::profile_scope!("dlc");
                        for package in game.dlc {
                            let (rect, _) =
                                dlc.allocate_exact_size(vec2(400.0, 200.0), egui::Sense::click());
                            dlc.painter().rect_filled(
                                rect,
                                Rounding::same(4.0),
        ...
        if let Some(handle) = logo {
            ...
            let logo_rect = Rect::from_min_max(
                Pos2 {
                    x: (egui::lerp(hero_rect.min.x..=hero_rect.max.x - 180.0, frac2)),
                    y: (hero_rect.min.y),
                },
                Pos2 {
                    x: (egui::lerp(hero_rect.max.x..=hero_rect.max.x - 20.0, frac2)),
                    y: (egui::lerp(hero_rect.max.y..=hero_rect.min.y + 80.0, frac2)),
                },
            );
...
fn show_game_list_buttons(app: &mut MaximaEguiApp, ui: &mut Ui) {
    ...
    let icon_size = vec2(
        10. * app.game_view_bar.game_size,
        10. * app.game_view_bar.game_size,
    );
    ...
    ui.vertical(|ui| {
        ui.vertical(|filter_chunk| {
            ...
            filter_chunk.visuals_mut().widgets.inactive.fg_stroke =
...

            {
                ...
                filter_chunk.add_sized(
                    [260., 20.],
                    egui::text_edit::TextEdit::singleline(&mut app.game_view_bar.search_buffer)
                        .vertical_align(egui::Align::Center)
                        .hint_text(&app.locale.localization.games_view.toolbar.search_bar_hint),
                );
                filter_chunk.horizontal(|filters| {
                    let combo_width = 130.0 - filters.spacing().item_spacing.x;
                    enum_dropdown(
                        filters,
                        "GameTypeComboBox".to_owned(),
                        &mut app.game_view_bar.genre_filter,
                        combo_width,
                        "",
                        &app.locale,
                    )
                    ...
                    enum_dropdown(
                        filters,
                        "PlatformComboBox".to_owned(),
                        &mut app.game_view_bar.platform_filter,
                        combo_width,
                        "",
                        &app.locale,
                    )
        ...
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                ...
                ui.vertical(|games_list| {
                    ...
                    let mut games: Vec<(&String, &GameInfo)> = games
                        .filter(|obj| {
                            matcher
                                .fuzzy_match(&obj.1.name, &app.game_view_bar.search_buffer)
                                .is_some()
                        })
                        .collect();
                    games.sort_by(|(_, a_game), (_, b_game)| a_game.name.cmp(&b_game.name));
                    ...
                    for (slug, game) in games {
                        ...
                        let x = if let Some(running_slug) = &app.playing_game {
                            running_slug.eq(slug)
                        } else {
                            false
                        };
                        let name = if x {
                        ...
                        let list_response = games_list.add_sized(
                            vec2(250.0, icon_size.y + 4.0),
                            egui::Button::image_and_text(
                                (egui::TextureId::Managed(0), vec2(0.0, 0.0)),
                                name,
                            )
...
pub fn games_view(app: &mut MaximaEguiApp, ui: &mut Ui) {
    ...
    if app.games.len() < 1 {
        ...
        let list_rect = Rect {
            ...
            max: pos2(
                ui.available_rect_before_wrap().min.x + list_width,
                ui.available_rect_before_wrap().max.y,
            ),
        };
        let game_rect = Rect {
            min: ui.available_rect_before_wrap().min
                + vec2(list_width + ui.spacing().item_spacing.x, 0.0),
